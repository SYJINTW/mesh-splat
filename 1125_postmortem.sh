...
BUDGETS=(1280000 640000 320000 160000 80000 40000 1) 

POLICIES=("planarity1" "planarity2" "planarity3" "distortion_no_avg" )
...
MESH_FILE="/mnt/data1/samk/NEU/sorted_dataset/milo_meshes/${SCENE_NAME}-dw50/${SCENE_NAME}-dw50.ply"
...
IS_WHITE_BG="--white_background" # set to "--white_background" if the dataset has white background
    ...
    for policy in "${POLICIES[@]}"; do
        for budget in "${BUDGETS[@]}"; do
            ...
            if python train.py --eval \
                ...
                $IS_WHITE_BG \
                $RESOLUTION \
                --iteration 10 2>&1 >> "$LOG_FILE" ; then
            ...
            if [ "$exp_status" = "WARMUP_SUCCESS" ]; then
                ...
                if python train.py --eval \
                    ...
                    $IS_WHITE_BG \
                    $RESOLUTION \
                    --iteration "$ITERATION" 2>&1 >> "$LOG_FILE"; then
            ...
            # ======= Step 2: Render ======
            render_success=true
            for iter in 1000 2000 3000 4000 5000 6000 7000 10000 12000 15000 $ITERATION; do
                if [ "$exp_status" = "TRAIN_SUCCESS" ]; then
                    ...
                    if python render_mesh_splat.py \
                        ...
                        $IS_WHITE_BG \
                        --policy_path "$POLICY_CACHED" \
                        --iteration "$iter" 2>&1 >> "$LOG_FILE" ; then
            ...
            if [ "$exp_status" = "RENDER_SUCCESS" ]; then
                ...
                if python metrics.py \
                    -m "$SAVE_DIR" \
                    --gs_type gs_mesh  2>&1 >> "$LOG_FILE"; then
